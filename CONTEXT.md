...
**Cycle / Night-Day cycle** — One round of the game consisting of a night phase (secret role actions), then a discussion phase, then a voting phase. The game is multi-cycle (unlike One Night Ultimate Werewolf's single round), capped at a small number of cycles (3-4) to suit a 4-5 player lobby. Cycles are numbered starting at 1.

**Discussion phase** — The talk-only window that opens after each night, before voting. Ends on whichever comes first: a 1-minute timer, or every living player clicking ready (mirrors Briefing's early-exit pattern). Purely a discussion window — no voting happens here at all; that's the following Voting phase's job. Skipped entirely by Overtime, which goes straight to a forced vote.

**Voting phase** — Follows Discussion each cycle: players cast their elimination vote, capped at 1 minute, ending early once every living player has voted (votes are one-shot and can't be changed once cast, so an early end loses no discussion opportunity a still-changeable vote would have needed). Overtime's forced vote uses this same phase shape and timer, just with different tally rules (see Overtime round).
...
**Overtime round** — If the cycle cap is reached with no faction win condition met, the game skips both the night phase and the discussion phase, going straight to a forced sudden-death vote (every living player must vote, no abstaining; a tie still results in no elimination). If this still leaves no faction win condition met, the game ends in a draw (individual Serpent's Hand roles can still have independently met their personal win condition).
...
- *The Fool* (formerly "Jester") — wins if voted out by the Foundation during a voting-phase vote.
...
**The Tome** — Chaos Insurgency's shared item, held by exactly one living CI member at a time. Whoever holds it may, on any night, kill as their action instead of their normal ability (so any CI role can act as the killer while holding it, not just Infiltrator). It starts with the Infiltrator if one is in the game, else a random CI member. It passes two ways: automatically to a random living CI teammate if the current holder is eliminated, or manually — the current holder may hand it to a living CI teammate at any time during voting. Adapted from Town of Salem's Necronomicon (Coven expansion), simplified for a CI team that's usually only 1-2 people: no group vote to decide the hand-off, just the current holder's own choice.
...
**Phase timer** — Night phase (role actions) uses a soft timer (advances once all required actions are submitted, no hard cutoff). Discussion and Voting each use a hard 1-minute timer with their own early-exit condition (see their glossary entries) — whichever comes first, the timer or the exit condition.
